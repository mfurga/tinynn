...
class TestDot(unittest.TestCase):
    ...
    def test_dot_2d_1d(self):
        x = Tensor(
            [[1, -12, 49, -42], [0, 9, -3, 0], [41, 0, -13, 3]], requires_grad=True
        )
        ...
        z.backward()
        assert x.grad.equals(
            Tensor([[-5, 3, 14, -25], [-5, 3, 14, -25], [-5, 3, 14, -25]])
        )
    ...
    def test_dot_1d_2d(self):
        ...
        y = Tensor(
            [[1, -12, 49], [0, 9, -3], [41, 0, -13], [2, -3, 13]], requires_grad=True
        )
        ...
        assert y.grad.equals(
            Tensor([[-5, -5, -5], [3, 3, 3], [14, 14, 14], [-25, -25, -25]])
        )

    def test_dot_2d_2d(self):
        x = Tensor([[7, 10, 4], [6, -9, 3]], requires_grad=True)

        y = Tensor(
            [[8, 9, -4, 13], [4, -1, 6, 10], [10, 47, -13, 1]], requires_grad=True
        )

        z = x.dot(y)
        assert z.equals(Tensor([[136, 241, -20, 195], [42, 204, -117, -9]]))

        z.backward()
        assert x.grad.equals(Tensor([[26, 19, 45], [26, 19, 45]]))
        assert y.grad.equals(Tensor([[13, 13, 13, 13], [1, 1, 1, 1], [7, 7, 7, 7]]))
